...
func (h *RequestHandler) getForumDetails(ctx *fasthttp.RequestCtx) {
	...
	log.Printf("/forum/{slug}/details", slug)
...
func (h *RequestHandler) getForumUsers(ctx *fasthttp.RequestCtx) {
	...
	log.Printf("/forum/{slug}/users", limit, since, desc)
...
func (h *RequestHandler) getForumThreads(ctx *fasthttp.RequestCtx) {
	...
	log.Printf("/forum/{slug}/threads", slug, limit, since, desc)
...
func (h *RequestHandler) getThreadDetails(ctx *fasthttp.RequestCtx) {
	...
	log.Printf("/thread/{slug_or_id}/details", slug)
...
func InsertCreateForumThread(p *pgxpool.Pool, thread Thread) (int, []byte) {
	...
	forumId = 0
	rows, err = conn.Query(context.Background(), "SELECT id, slug FROM public.forum WHERE lower(slug) = $1", strings.ToLower(thread.Forum))
	for rows.Next() {
	...
	rows, err = conn.Query(context.Background(),
		"INSERT INTO public.user_forum (nickname, slug) VALUES ($1, $2)",
		thread.Author, thread.Forum)
	rows.Close()

	rows, err = conn.Query(context.Background(), "UPDATE public.forum SET threads = threads + 1 WHERE id = $1", forumId)
	rows.Close()
...
func getForumThreads(p *pgxpool.Pool, slug string, limit string, since string, desc string) (int, []byte) {
	...
	var rows pgx.Rows
	var forumId uint64
	rows, err = conn.Query(context.Background(), "SELECT id FROM public.forum WHERE lower(slug) = $1", strings.ToLower(slug))
	for rows.Next() {
		rows.Scan(&forumId)
	}
	rows.Close()
...
func getForumUsers(p *pgxpool.Pool, slug string, limit string, since string, desc string) (int, []byte) {
	...
	rows, err = conn.Query(context.Background(), "SELECT users.nickname, users.about, users.email, users.fullname FROM user_forum JOIN users ON users.nickname = user_forum.nickname WHERE lower(slug) = $1", strings.ToLower(slug))
	for rows.Next() {
		var user User
...
func getThreadDetails(p *pgxpool.Pool, slug string) (int, []byte) {
	...
	if flag {
		err = conn.QueryRow(context.Background(), "SELECT users_nickname, created, id, message, title, votes, slug, forum FROM public.thread WHERE lower(slug) = $1", strings.ToLower(slug)).Scan(&thread.Author, &thread.Created, &thread.Id, &thread.Message, &thread.Title, &thread.Votes, &slugInter, &thread.Forum)
	} else {
		err = conn.QueryRow(context.Background(), "SELECT users_nickname, created, id, message, title, votes, slug, forum FROM public.thread WHERE id = $1", selector).Scan(&thread.Author, &thread.Created, &thread.Id, &thread.Message, &thread.Title, &thread.Votes, &slugInter, &thread.Forum)
	}
...
func addPostThread(p *pgxpool.Pool, slugOrId string, posts Posts) (int, []byte) {
	...
	currentTime = currentTime.Round(time.Microsecond)
	...
	for i := 0; i < len(posts); i++ {
		...
		rows, err = conn.Query(context.Background(),
			"INSERT INTO public.user_forum (nickname, slug) VALUES ($1, $2)",
			posts[i].Author, thread.Forum)
		rows.Close()
	}

	rows, err = conn.Query(context.Background(), "UPDATE public.forum SET posts = posts + $2 WHERE lower(slug) = $1", strings.ToLower(thread.Forum), len(posts))
	rows.Close()
